...
function displayBoard() {
  ...
  for (let row = 0; row < BOARD.length; row++) {
    for (let col = 0; col < BOARD[row].length; col++) {
      ...
      string += currentElemnt === "X" || currentElemnt === "O"
        ? color + currentElemnt + "\x1b[0m"
        : currentElemnt;
    }
    string += "\n";
  }
...
function isInValidInput(move) {
  const isNotValidInput = computerMoves.includes(move) ||
  ...
  for (let index = 0; index < REQ_PAIRS.length; index++) {
    if (REQ_PAIRS[index].includes(pair) && !invalidPairs.includes(pair)) {
      for (let char = 0; char < REQ_PAIRS[index].length; char++) {
        ...
        if (!pair.includes(REQ_PAIRS[index][char])) {
          return REQ_PAIRS[index][char];
        }
      }
    }
  }
...
function moveToNext(currentMove) {
  ...
  for (let index = 0; index < REQ_PAIRS.length; index++) {
    while (
      REQ_PAIRS[index].includes(move) && isInValidInput(+move) && !isCurrentMove
    ) {
...
function computerMove() {
  ...
  if (
    (isInValidInput(5) && computerMoves.length === 0) ||
  ...
  if (
    !isInValidInput(9) && computerMoves.length === 1 && isInValidInput(8) &&
    difficulty === 4
  ) {
    return 9;
  }
...
function playOneMove(currentPlayer, players) {
  ...
  const move = currentPlayer === 1
    ? +computerMove()
    : parseInt(prompt(`${players[currentPlayer]} turn => `));
  if (isInValidInput(move)) {
...
function play(players, mode) {
  ...
  const color = (currentMove + whosGoingFirst) % 2 === 1
    ? "\x1b[91m"
    : "\x1b[36m";
...
function namePlayers(mode) {
  ...
  const blue = mode === 1
    ? "COMPUTER"
    : prompt("Enter the name of the player 2:");
  return [red, blue];
}
...
function selectDifficulty() {
  ...
  const respone = parseInt(
    prompt("1. Easy\n2. Medium\n3. Hard\n4. Impossible\n>> "),
  );
  return respone;
}

function selectPairs(difficulty) {
  switch (difficulty) {
    case 1:
      return EASY_PAIRS;
    case 2:
...
export function ticTacToe() {
  ...
  REQ_PAIRS = selectPairs(difficulty);
  // ticTacToe();
  instructionBoard();
